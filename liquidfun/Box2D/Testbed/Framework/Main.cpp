...
namespace
{
	...
	int _framesToRun = 100;
	const int MAX_FRAMES_TO_RUN_HEADLESS = 10000;
}

static void _init_glut(int argc, char** argv);

bool IsHeadless() {
	return _headless;
}

void EnableHeadlessMode(int framesToRun = 10, int testToRun = 4) {
	_framesToRun = framesToRun;
	testSelection = testToRun;
}
...
static void SimpleUncappedMainLoop()
{
	while (_framesToRun-- > 0)
	{
		SimulationLoop();
		std::cout << "Steps remaining: " << _framesToRun << std::endl;
	}
}

static void _processArgs(int argc, char** argv)
{
	if (argc > 1 && !IsHeadless()) {
		std::string argv1(argv[1]);

		if (argv1 == "-headless") {
			_headless = true;
			int framesToRun = _framesToRun;
			int testToRun = testSelection;

			try {
				if (argc > 2) {
					framesToRun = std::atoi(argv[2]);
					if (framesToRun < 1 || framesToRun > MAX_FRAMES_TO_RUN_HEADLESS) {
						std::cout << "Invalid value. Steps to run headless will be set to " << _framesToRun << std::endl;
					}
					else {
						_framesToRun = framesToRun;
					}
				...
				if (argc > 3) {
					testToRun = std::atoi(argv[3]);
					if (testToRun < 1 || testToRun >= testCount) {
						std::cout << "Invalid value. Test to execute will be set to " << testSelection << std::endl;
					}
					else {
						testSelection = testToRun;
						testIndex = testToRun;
				...
				EnableHeadlessMode(framesToRun, testToRun);
			}
	...
	if (!IsHeadless()) {
		_init_glut(argc, argv);
	}
}

static void InitFramework(int argc, char** argv)
{
	testCount = 0;
...
{
	if (IsHeadless()) {
		SimpleUncappedMainLoop();
	}
	else {
		// Edited glut to return from main loop
		glutMainLoop();
	}
}
...
static void _init_glut(int argc, char** argv)
{
	...
	while (e->createFcn)
	{
		testList->add_item(_testCount, e->name);
		++_testCount;
		++e;
	}
...
// =============== DO NOT COMMIT CHANGES TO MAIN (see above) ======================
int main(int argc, char** argv)
{
	//"Headless" will run the application without the GUI for testing purposes.
	//EnableHeadlessMode();

	//Initializes our the testbed, glut, and processes command line args
	InitFramework(argc, argv);

	//This will kick-start the application
	//This project relies on glut. Normally, glut expects to never return  
	// from its main loop, but we've slightly modified our version to return
	// to allow us to rely on object destructors if we choose to use them.
	Run();

	//This will let glut exit gracefully if not running as headless
	// This calls glutExit(), which calls Exit(0), 
	// so nothing after this call will be executed
	// Anything you need to create should go in Run() above
	OnExit();
